...
def test_polynomial(fit_model):
    ...
    formula = (
        ronia.Scalar(prior=(0, 1e-6)) * x +
        ronia.Scalar(prior=(0, 1e-6)) * x ** 2 +
        ronia.Scalar(prior=(0, 1e-6))
    )
    model = fit_model(formula, ronia.numpy.Delta(0.01342))(input_data, y)
    assert_almost_equal(
...
def test_gp(fit_model):
    ...
    formula = ronia.ExpSineSquared1d(
        np.arange(0, 2 * np.pi, 0.1),
        corrlen=1.0,
        sigma=1.0,
        period=2 * np.pi,
        energy=0.99
    )(x[:, 0]) * x[:, 1] + ronia.Scalar(prior=(0, 1e-6))
    model = fit_model(formula, ronia.numpy.Delta(100))(input_data, y)
    assert_almost_equal(
...
def test_kron(fit_model):
    ...
    model = fit_model(formula, ronia.numpy.Delta(24.2289))(input_data, y)
    assert_almost_equal(
...
def test_bspline(fit_model):
    ...
    model = fit_model(formula, ronia.numpy.Delta(0.0136602))(input_data, y)
    assert_almost_equal(
